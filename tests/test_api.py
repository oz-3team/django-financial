...
class TestUserAPI:
    def test_signup(self, api_client, user_data):
        url = reverse("signup")
        response = api_client.post(url, user_data, format="json")
        assert response.status_code == status.HTTP_201_CREATED
        user = CustomUser.objects.get(email=user_data["email"])
        assert not user.is_active  # 이메일 인증 전 비활성 상태

    def test_email_verification(self, api_client, user_data):
        # 회원가입 후 이메일 인증 절차
        signup_url = reverse("signup")
        api_client.post(signup_url, user_data, format="json")
        user = CustomUser.objects.get(email=user_data["email"])
    ...
    def test_login(self, api_client, user_data):
        # 회원가입과 활성화를 미리 처리한 유저 생성
        CustomUser.objects.create_user(
            email=user_data["email"], password=user_data["password"], is_active=True
        )
    ...
    def test_profile(self, api_client, user_data):
        tokens = self.test_login(api_client, user_data)
